...
Focused DPE-ARCH-0016 evidence (2026-07-29): public Qt interaction tests exercise Back/Forward/Up, clickable breadcrumbs, active-folder and expansion restoration after model replacement, synchronized stable selection, and both the new no-Scene Project move path and existing Scene/Hierarchy/Inspector drop path. The Project Model regression proves deterministic folder-first natural ordering. Complete local Windows Release passes 63/63 in 510.19 seconds, MSVC AddressSanitizer passes 63/63 in 717.87 seconds without a sanitizer finding, and Ubuntu 24.04 Release passes 62/62 in 152.01 seconds. macOS, complete keyboard/accessibility/high-DPI/high-contrast inspection, distribution materials, and legal review remain open; ADR-0007 remains `Proposed`.
