...
Focused DPE-ARCH-0016 evidence (2026-07-29): the public Project drop regression removes the active Scene, moves one stable sprite asset and one nested folder through `AssetService`, refreshes the index, and verifies the original stable identities at their new contained paths. The existing domain-drop regression still covers sprite/Tilemap/prefab creation or assignment through Scene, Hierarchy, Inspector, and prefab owners plus stale/cross-project rejection. Windows Release passes 63/63, MSVC AddressSanitizer passes 63/63 without findings, and Ubuntu Release passes 62/62. This evidence introduces no second mutation path or Undo owner and does not complete the broader command/automation/POC H gates; ADR-0008 remains `Proposed`.
