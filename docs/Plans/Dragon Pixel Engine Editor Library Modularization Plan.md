...
The implementation and first verification run on Windows 11 x64 because the current complete baseline and build outputs are present there. The target structure is portable CMake and must not add a Windows-only source or link assumption. Focused Windows Release/ASan evidence does not close the current Ubuntu or macOS matrices, POC H, an ADR, or a slice.

PR #3 CI attempted all six platform/configuration jobs. Windows Release and MSVC ASan pass. Both macOS jobs stop in unchanged `src/native/Metadata/src/registry.cpp` before any editor-library object compiles because AppleClang 17 reports 16 missing-field-initializer warnings as errors. Both Ubuntu jobs stop before project configuration because the vcpkg `libxcrypt` build cannot find the `Ninja Multi-Config` build program. These are current platform/harness blockers outside this feature's nine-file scope, not editor-library pass or fail evidence; they remain for the documented platform/CI follow-up rather than being absorbed into this branch.
