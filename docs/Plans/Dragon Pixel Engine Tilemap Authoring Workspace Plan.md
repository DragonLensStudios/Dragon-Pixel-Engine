...
| 2026-07-28 | Brush fixture portability correction implemented | Hosted run `30357735329` compiled the complete production editor library on all four Unix configurations, then macOS reached `EditorInteractionTests.cpp` and rejected four remaining test-only partial Brush initializers, including two anonymous `select_brush` arguments missed by the type-name scan. Commit `416c911` gives each fixture an explicit Brush record with the same asserted fields and defaults. The complete `s2.editor_interactions` alias passes in **155.87 seconds Release** and **218.30 seconds MSVC AddressSanitizer**. Production-bundle generation on the preceding production-equivalent head completed the packaged MonoGame smoke test; all **189** manifest records match size/SHA-256 with zero missing, mismatched, or unlisted files. The final exact-head bundle and aggregate matrices remain to be reconciled after hosted compilation stabilizes. |
| 2026-07-28 | POSIX recovery-fixture scope corrected | Hosted run `30358797199` compiled every Windows, Ubuntu, and macOS target without a feature-owned warning. The dedicated Tilemap creation workflow passed on macOS and Windows, while the broad jobs remained red on existing graphics-device, worker-frame/crash, runtime-manifest, macOS temporary-path/Rider, ASan-interceptor, and interaction/POC J platform gates. Ubuntu additionally showed that the new scene-plus-Tilemap recovery fixture expected the Windows-only persistent delete-sharing fault to fail even though that injected seam is deliberately inert on POSIX. Commit `7159b1f` scopes only the Win32 sharing-message subcase to Windows; the cross-platform interrupted publication, rollback, dirty-state, retry, and save/reopen checks remain active. The exact method passes under the configured Qt runtime in strict Windows Release and MSVC AddressSanitizer; an earlier raw launch without Qt DLLs exited before assertions and is inconclusive. Immediately before this two-directive test-only guard, the exact branch passed **63/63 in 507.12 seconds Release** and **63/63 in 686.85 seconds MSVC AddressSanitizer**. |
