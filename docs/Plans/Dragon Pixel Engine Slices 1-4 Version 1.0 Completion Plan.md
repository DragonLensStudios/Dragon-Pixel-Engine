...
| --- | --- | --- | --- |
| Strict Release | CI/GitFlow feature branch passes 56/56 locally in 410 s with validated JUnit; hosted feature-PR result pending | PR #3 failed before tests on Metadata aggregate warnings; explicit-initialization repair implemented, hosted result pending | PR #3 failed before configure on missing autotools; hosted/container prerequisites implemented, hosted result pending |
| Native AddressSanitizer | CI/GitFlow feature branch passes 56/56 locally in 551 s with validated JUnit; hosted feature-PR result pending | PR #3 failed before tests on the same Metadata warnings; explicit-initialization repair implemented, hosted result pending | PR #3 failed before configure on the same provisioning blocker; hosted/container prerequisites implemented, hosted result pending |
