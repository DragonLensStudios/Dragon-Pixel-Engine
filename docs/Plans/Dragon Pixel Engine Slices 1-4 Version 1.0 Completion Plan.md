...
| --- | --- | --- | --- |
| Strict Release | Current `develop` after merged atomic repair passes 56/56 in 423.22 s; editor-library PR #3 CI passes | PR #3 current run fails before tests on unchanged Metadata missing-field-initializer warnings under AppleClang 17 `-Werror` | PR #3 current run fails before project configure when vcpkg cannot locate `Ninja Multi-Config` while building `libxcrypt` |
| Native AddressSanitizer | Current `develop` after merged atomic repair passes 56/56 in 560.43 s; editor-library PR #3 CI passes | PR #3 current run fails before tests on the same unchanged Metadata warnings | PR #3 current run fails before project configure on the same Ninja/vcpkg provisioning blocker |
