...
| 2026-07-27 | TileSet PNG intake correction verified | On the same active `feature/tilemap-authoring-workspace` branch and draft PR #6, a reported valid extensionless PNG was confirmed to have the PNG signature while Create rejected only its missing suffix. Preview and creation now share content-based PNG decoding, preserve the validated bytes into a normalized contained `.png`, expose the full selected path, and reject missing or mislabeled non-PNG input before output. The expanded affected workflow passes **8/8 in 37.941 seconds** under Release and **8/8 in 63.733 seconds** under MSVC AddressSanitizer; the complete strict Release preset passes **60/60 in 482.94 seconds**. The 189-record developer bundle hash-verifies with zero inventory differences and its packaged MonoGame self-test passes. The broader ASan preset was not repeated; its previously recorded 59/60 result and unchanged POC J performance blocker remain open. No format, ABI, protocol, support, platform, POC, ADR, slice, release, or KNI status changed. |
| 2026-07-27 | Paint-ready Tilemap workflow verified | On the active `feature/tilemap-authoring-workspace` branch and draft PR #6, **Create TileSet from Image...** now offers a default-on complete workflow that creates the dependent Tilemap, reuses a selected blank Tilemap2D or creates an assigned preset, selects it, enters the 2D workspace, and exposes the first layer/brush. Double-clicking an unmapped existing TileSet offers the same path; open-map presets inherit the map; unavailable scene attachment retains the published asset. The exact workflow covers Undo/Redo, painting, save/reopen, and recovery. Affected Release passes 14/14 and ASan passes 15/15; the isolated final workflow passes in 19.08 seconds Release and 33.11 seconds ASan. The complete strict Release preset passes **61/61 in 528.41 seconds** under unchanged limits. The production bundle rebuild remains blocked by the user's open editor (PID 56444), the complete ASan preset was not repeated, and all platform/POC/ADR/slice/release/KNI limitations remain unchanged. |
