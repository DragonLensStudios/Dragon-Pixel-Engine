...
Dragon Pixel Engine is pursuing the complete four-slice roadmap and design-defined `1.0.0` under the active mirrored master plan. Editor-library modularization PR #3, CI/GitFlow PR #4, isolated Tiled-import PR #5, and DPE-ARCH-0015 Tilemap PR #6 are merged into `develop`. The current DPE-ARCH-0016 Project View branch passes **63/63** strict Windows Release tests in **510.19 seconds**, **63/63** MSVC AddressSanitizer tests in **717.87 seconds** without a sanitizer finding, and **62/62** Ubuntu 24.04 Release tests in **152.01 seconds**. macOS is deferred for this feature and remains an open product gate, so no complete cross-platform, POC, ADR, slice, release, or KNI production claim is made. KNI remains experimental.
...
The **Project** dock uses a folder tree on the left and the active folder's immediate contents on the right. Back, Forward, Up, and clickable breadcrumb controls provide stable folder navigation. List and tile modes share one deterministic folder-first natural ordering and synchronize stable selection. Refresh restores the nearest valid folder, expansion, selection, splitter position, and view mode as per-user editor state without changing project files.
