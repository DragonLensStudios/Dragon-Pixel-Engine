...
The current synchronized design revision is `DPE-ARCH-0014`, last reviewed 2026-07-27. For maintainers and project agents with access to both roots, if either location is unavailable, paired hashes/revisions differ, or a file appears incomplete, stop documentation, planning, and implementation work and repair or clarify the mirror first. Neither path, timestamp, nor Git status automatically wins a conflict. An external contributor who cannot access the private documentation-system root may prepare a repository-only proposal only when the proposal clearly states that the external mirror was unavailable and unchecked; that proposal is not accepted project evidence and may not merge until a maintainer synchronizes and byte-verifies every affected pair.
...
The current CI/GitFlow branch makes native commands fail closed, validates GitFlow relationships and JUnit evidence, pins runner build dependencies, and removes the strict Clang/AppleClang initialization blockers without changing runtime values. Platform-correct evidence floors are 56 tests on Windows and 55 on Unix because Unix intentionally excludes Windows launcher probes. Local Windows passes 56 of 56 in 415.61 seconds Release and 533.20 seconds MSVC AddressSanitizer. Hosted run `30303135271` passes Ubuntu Release 55 of 55 in 155.34 seconds; Ubuntu ASan passes 50 of 55, macOS Release and ASan pass 44 of 55 and 41 of 55, and hosted Windows Release and ASan pass 44 of 57. All jobs build and run real tests with JUnit/failure artifacts; remaining reds are product/platform evidence rather than CI harness failures. The local Docker daemon remains unavailable, and branch protection is unmodified. The merged atomic-repair `s1.native_core` stability evidence remains 100 of 100 Release and 50 of 50 sanitizer repetitions, and the aggregate sanitizer interaction aliases remain under unchanged 480-second caps. ADR-0006 handle-pinning/noncooperating-writer limits remain open. Complete POC J acceptance remains open, no complete current POSIX matrix passes, macOS rendering/interaction failures remain, POCs M-S remain open with only focused Windows portions of M/O implemented, every ADR remains Proposed, and KNI remains experimental.
